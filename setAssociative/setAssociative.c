...
void accessData (
    ...
    mem_addr_t x = addr; 
    int index = (addr & 0x3);
    mem_addr_t tag = x >> 2; //tag 

    // Find the correct cache set
    cache_set_t set = (*cache)[index]; 

    // Cache hit
    for(int i=0; i<4; i++){ //number of lines per set is 4
        if(set[i].tag == tag && set[i].valid==true){
            (*hit_count)++; 
            set[i].recent_use=program_counter;
            return; 
        }
    }
    ...
    for(int j=0; j<4; j++){
        if (set[j].recent_use < temp) {
            setIndex = j; 
            temp = set[j].recent_use;
        }
    }


    // If cache set line already in use, then record an eviction
    if (set[setIndex].valid==true){
        (*eviction_count)++; 
    }

    set[setIndex].recent_use=program_counter; 
    set[setIndex].tag=tag;
    set[setIndex].valid=true; 
}
