...
void accessData (
    ...
    unsigned char occ = cache->occupancy; 
    ...
    if ( cache->occupancy == E ) {
        ...
        if (cache->front!=NULL) { // If the Queue is currently empty
            ...
            //cache_line_t* next = temp->next_cache_line;

            cache->front = temp->next_cache_line;
            if (cache->back==temp) { // If the Queue will become empty
                cache->back = NULL;
            }
    ...
    cache->occupancy=occ; 
        ...
        cache_line_t* new_line = malloc(sizeof(cache_line_t));
        new_line -> tag = tag;
        new_line -> next_cache_line = NULL; // At back of the queue, there is no next node.
