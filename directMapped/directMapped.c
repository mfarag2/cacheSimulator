...
void accessData (
    ...
    int ind1 = (addr & 0xf);
    int temp = ind1; 
    mem_addr_t tag = x >> 4; //tag 

    // Cache hit  
   
   if ((*cache)[temp].tag == tag && (*cache)[temp].valid==true){
       (*hit_count)++; 
       return; 
   }
...
int main(int argc, char* argv[]) {
    ...
    for (size_t set_index=0; set_index<16; set_index++) {
        cache[set_index].tag=0;
        cache[set_index].valid=false; 
    }
